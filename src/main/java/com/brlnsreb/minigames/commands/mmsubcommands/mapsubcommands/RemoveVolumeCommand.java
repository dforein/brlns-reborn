...
public class RemoveVolumeCommand extends SubCommand {
    ...
    public RemoveVolumeCommand(MurderMysteryGame game) {
        super("removevolume");
    ...
    @Override
	public boolean execute(CommandSender sender, String arcommandLabelg1, String[] args) {

        Player player = (Player) sender;

        if (args.length < 4) {
            player.sendMessage(TextFormat.RED + "Usage: /mm map remove <mapId> <from> <to>");
            player.sendMessage(TextFormat.RED + "Usage: /mm map remove <mapId> savecurrentpos");
            player.sendMessage(TextFormat.RED + "Usage: /mm map remove <mapId> usesavedpos");
            return true;
        }
        ...
        if (args[3].equalsIgnoreCase("usesavedpos")) {
            if (position1 != null && position2 != null) {
                game.getMapper().removeVolume(mapId, position1, position2, player);
            } else {
        ...
        try {
            ...
            game.getMapper().removeVolume(mapId, pos1, pos2, player);
